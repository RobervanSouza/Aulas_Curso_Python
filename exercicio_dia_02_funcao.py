...
def sum_two_lists(abobora, cenoura):
  return len(abobora) + len(cenoura)

def join_two_lists(abelha, arapua):
    return abelha + arapua
def get_the_biggest(abobora):
    return max(abobora)

def count_in_list(beterraba, in_verduras):
    return in_verduras.count(beterraba)
